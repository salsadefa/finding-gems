...
import { sendPayoutRequestedEmail, sendPayoutProcessedEmail } from '../services/email.service';
 ...
 */
export const getCreatorBalance = catchAsync(async (req: Request, res: Response) => {
 ...
 */
export const recalculateBalance = catchAsync(async (req: Request, res: Response) => {
  const user = (req as any).user;
  if (!user) {
    return res.status(401).json({ success: false, error: { message: 'Unauthorized' } });
  }
 ...
 * GET /api/v1/payouts
 */
export const getPayouts = catchAsync(async (req: Request, res: Response) => {
  const user = (req as any).user;
  if (!user) {
    return res.status(401).json({ success: false, error: { message: 'Unauthorized' } });
  }
...
export const requestPayout = catchAsync(async (req: Request, res: Response) => {
  ...
  const { data: payoutNumResult } = await supabase.rpc('generate_payout_number');
  ...
  if (user.email) {
    sendPayoutRequestedEmail(user.email, {
 ...
 */
export const cancelPayout = catchAsync(async (req: Request, res: Response) => {
  const user = (req as any).user;
  if (!user) {
    return res.status(401).json({ success: false, error: { message: 'Unauthorized' } });
  }
...
export const processPayout = catchAsync(async (req: Request, res: Response) => {
  ...
  if (creator?.email) {
    sendPayoutProcessedEmail(creator.email, {
 ...
 */
async function calculateCreatorBalance(creatorId: string) {
