 ...
 */
export const getRefundDetail = catchAsync(async (req: Request, res: Response) => {
  const user = (req as any).user;
  if (!user) {
    return res.status(401).json({ success: false, error: { message: 'Unauthorized' } });
  }
 ...
 */
export const cancelRefund = catchAsync(async (req: Request, res: Response) => {
  const user = (req as any).user;
  if (!user) {
    return res.status(401).json({ success: false, error: { message: 'Unauthorized' } });
  }
...
export const processRefund = catchAsync(async (req: Request, res: Response) => {
  ...
  if (requester?.email && ['approved', 'rejected', 'completed'].includes(newStatus)) {
    const order = refund.order as any;
    sendRefundStatusEmail(requester.email, {
      userName: requester.name || 'Customer',
      refundNumber: refund.refund_number,
      orderNumber: order?.order_number || 'N/A',
      amount: Number(refund.refund_amount),
      status: newStatus as 'approved' | 'rejected' | 'completed',
      message: status_message
    }).catch((err: Error) => console.error('Failed to send refund status email:', err));
  }
