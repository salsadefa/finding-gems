...
export const initiatePayment = catchAsync(async (req: Request, res: Response) => {
  ...
  const useXendit = xenditService.isAvailable() && payment_method !== 'manual';
  ...
  if (useXendit) {
    try {
      ...
      xenditResponse = await xenditService.createInvoice({
        orderId: order_id,
...
export const getPaymentStatus = catchAsync(async (req: Request, res: Response) => {
  ...
  if (transaction.payment_provider === 'xendit' && transaction.gateway_transaction_id) {
    try {
      const xenditStatus = await xenditService.getInvoiceStatus(transaction.gateway_transaction_id);
      const mappedStatus = xenditService.mapStatus(xenditStatus.status);
...
export const handleXenditWebhook = catchAsync(async (req: Request, res: Response) => {
  ...
  const payload = xenditService.parseWebhookPayload(req.body);
  ...
  const mappedStatus = xenditService.mapStatus(payload.status);
 ...
 */
export const handlePaymentWebhook = catchAsync(async (req: Request, res: Response, next) => {
  // Check if it's a Xendit webhook
  if (req.headers['x-callback-token']) {
    return handleXenditWebhook(req, res, next);
  }
 ...
 */
async function grantAccessAndCreateInvoice(orderId: string) {
  ...
  if (order.buyer?.email) {
    sendPaymentSuccessEmail(order.buyer.email, {
    ...
    // Also send invoice
    if (invoice) {
      sendInvoiceEmail(order.buyer.email, {
        userName: order.buyer.name || 'Customer',
  ...
  if (order.creator?.email) {
    ...
    sendNewSaleEmail(order.creator.email, {
 ...
 */
function getPaymentInstructions(paymentMethod: string, amount: number, transactionId: string) {
