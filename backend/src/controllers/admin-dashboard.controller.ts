...
export const getDashboardOverview = catchAsync(async (req: Request, res: Response) => {
  ...
  const [
    ...
    // Pending payouts
    supabase.from('payouts').select('id, amount', { count: 'exact' }).eq('status', 'pending'),
...
export const getPaymentAnalytics = catchAsync(async (req: Request, res: Response) => {
  ...
  // Group by date
  const dailyData: Record<string, { revenue: number; orders: number; fees: number }> = {};
 ...
 */
export const getTopPerformers = catchAsync(async (req: Request, res: Response) => {
  const user = (req as any).user;
  if (!user || user.role !== 'admin') {
    return res.status(403).json({ success: false, error: { message: 'Admin access required' } });
  }
  ...
  const { data: topWebsites } = await supabase
    .from('orders')
    .select(`
      website_id,
  ...
  const websiteSales: Record<string, { website: any; revenue: number; orders: number }> = {};
  ...
  const creatorEarnings: Record<string, { creator: any; revenue: number; earnings: number; orders: number }> = {};
