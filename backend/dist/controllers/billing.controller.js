...
exports.getCreatorSales = exports.checkAccess = exports.getMyAccess = exports.getMyInvoices = exports.getOrderInvoice = exports.cancelOrder = exports.getMyOrders = exports.getOrder = exports.createOrder = exports.deletePricingTier = exports.updatePricingTier = exports.createPricingTier = exports.getWebsitePricing = void 0;
 ...
 */
exports.getWebsitePricing = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const { websiteId } = req.params;
    const { data: tiers, error } = await supabase_1.supabase
        .from('pricing_tiers')
        .select('*')
 ...
 */
exports.createPricingTier = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const user = req.user;
    if (!user) {
    ...
    // Create pricing tier
    const { data: tier, error } = await supabase_1.supabase
        .from('pricing_tiers')
        .insert({
 ...
 */
exports.updatePricingTier = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const user = req.user;
    if (!user) {
        return res.status(401).json({ success: false, error: { message: 'Unauthorized' } });
    }
    ...
    const { data: tier, error: tierError } = await supabase_1.supabase
        .from('pricing_tiers')
    ...
    if (tier.websites.creatorId !== user.id && user.role !== 'admin') {
 ...
 */
exports.deletePricingTier = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const user = req.user;
    if (!user) {
        return res.status(401).json({ success: false, error: { message: 'Unauthorized' } });
    }
...
exports.createOrder = (0, catchAsync_1.catchAsync)(async (req, res) => {
    ...
    const { website_id, pricing_tier_id, notes } = req.body;
    if (!website_id) {
    ...
    const { data: existingAccess } = await supabase_1.supabase
        .from('user_access')
        .select('id')
        .eq('user_id', user.id)
    ...
    const { data: existingOrders, error: orderCheckError } = await supabase_1.supabase
        .from('orders')
    ...
    if (pricing_tier_id) {
        const { data: tier } = await supabase_1.supabase
            .from('pricing_tiers')
            .select('*')
        ...
        const { data: defaultTier } = await supabase_1.supabase
            .from('pricing_tiers')
            .select('*')
            .eq('website_id', website_id)
            .eq('is_active', true)
    ...
    const { data: orderNumResult } = await supabase_1.supabase.rpc('generate_order_number');
...
exports.getOrder = (0, catchAsync_1.catchAsync)(async (req, res) => {
    ...
    const { data: invoice } = await supabase_1.supabase
        .from('invoices')
        .select('*')
        .eq('order_id', orderId)
        .single();
 ...
 */
exports.getOrderInvoice = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const user = req.user;
    if (!user) {
        return res.status(401).json({ success: false, error: { message: 'Unauthorized' } });
    }
 ...
 */
exports.getMyInvoices = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const user = req.user;
    if (!user) {
        return res.status(401).json({ success: false, error: { message: 'Unauthorized' } });
    }
 ...
 */
exports.getMyAccess = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const user = req.user;
    if (!user) {
        return res.status(401).json({ success: false, error: { message: 'Unauthorized' } });
    }
    const { data: access, error } = await supabase_1.supabase
        .from('user_access')
        .select(`
      *,
 ...
 */
exports.checkAccess = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const user = req.user;
    if (!user) {
        return res.status(401).json({ success: false, error: { message: 'Unauthorized' } });
    }
    const { websiteId } = req.params;
    const { data: access } = await supabase_1.supabase
        .from('user_access')
        .select('*')
        .eq('user_id', user.id)
        .eq('website_id', websiteId)
 ...
 */
exports.getCreatorSales = (0, catchAsync_1.catchAsync)(async (req, res) => {
    const user = req.user;
    ...
    // Calculate totals
    const { data: totals } = await supabase_1.supabase
        .from('orders')
